...
string GetRecharge(string ID) {
	...
	FILE* fp = fopen("D:\\Winter_temp\\recharge.txt", "a+");
	fscanf(fp, "%s\n", buffer);
	int i = 0, j = 0;
	while (!feof(fp)) {
		fscanf(fp, "%s\n", buffer);
...
void User::init() {
	all.clear();
	bool empty = false;
	FILE* buf = fopen("D:\\Winter_temp\\user.txt", "a+");
	assert(buf != NULL && "文件打开失败\n");
	fclose(buf);
	FILE* p = fopen("D:\\Winter_temp\\user.txt", "r");
	assert(p != NULL && "user文件打开失败");
	char f[100];
	fscanf(p, "%s", f);
	if (feof(p)) empty = true;;
	fclose(p);
	FILE* fp = fopen("D:\\Winter_temp\\user.txt", "a+");
	assert(fp != NULL && "user文件打开失败\n");
	if (empty) fprintf(fp, "用户ID,用户名,密码,联系方式,地址,钱包余额,用户状态\n");
	else {
		char buffer[200]; fscanf(fp, "%s\n", buffer);
...
void User::preserve() {
	FILE* fp = fopen("D:\\Winter_temp\\user.txt", "w+");
...
void Buyer::recharge() {
	...
	if (check_num(num)) {
		bool empty = false;
		FILE* buf = fopen("D:\\Winter_temp\\recharge.txt", "a+");
		assert(buf != NULL && "文件打开失败\n");
		fclose(buf);
		FILE* p = fopen("D:\\Winter_temp\\recharge.txt", "r");
