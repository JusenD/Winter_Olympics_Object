...
void Commodity::init() {
	all.clear();
	bool empty = false;
	FILE* buf = fopen("D:\\Winter_temp\\commodity.txt", "a+");
	assert(buf != NULL && "文件打开失败\n");
	fclose(buf);
	FILE* p = fopen("D:\\Winter_temp\\commodity.txt", "r");
	assert(p != NULL && "user文件打开失败");
	char f[100];
	fscanf(p, "%s", f);
	if (feof(p)) empty = true;;
	fclose(p);
	FILE* fp = fopen("D:\\Winter_temp\\commodity.txt", "a+");
...
void Commodity::preserve() {
	FILE* fp = fopen("D:\\Winter_temp\\commodity.txt", "w+");
	assert(fp != NULL && "文件打开失败\n");
...
void Order::init() {
	all.clear();
	bool empty = false;
	FILE* buf = fopen("D:\\Winter_temp\\order.txt", "a+");
	assert(buf != NULL && "文件打开失败\n");
	fclose(buf);
	FILE* p = fopen("D:\\Winter_temp\\order.txt", "r");
	assert(p != NULL && "user文件打开失败");
	char f[100];
	fscanf(p, "%s", f);
	if (feof(p)) empty = true;;
	fclose(p);
	FILE* fp = fopen("D:\\Winter_temp\\order.txt", "a+");
	assert(fp != NULL && "文件打开失败\n");
...
void Order::preserve() {
	FILE* fp = fopen("D:\\Winter_temp\\order.txt", "w+");
	assert(fp != NULL && "文件打开失败\n");
	fprintf(fp, "商品ID,名称,价格,数量,描述,卖家ID,上架时间,商品状态\n");
	int i = 0;
	while (i < all.size()) {
